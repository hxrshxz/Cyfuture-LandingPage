...
export default function TransactionResult({
  ...
  useEffect(() => {
    // Hide navbar elements by adding a class to body or by setting CSS variables
    document.body.style.setProperty("--modal-open", "1");
    ...
    // Cleanup function to show navbar when modal closes
    return () => {
      document.body.style.removeProperty("--modal-open");
      navElements.forEach((nav) => {
        nav.style.display = "";
      });
    };
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[10000] flex items-center justify-center p-4">
      <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto relative">
        <Card className="card-glow border-blue-500/20 shadow-2xl shadow-blue-500/10 relative">
          {/* Close Button */}
          <Button
            onClick={handleClose}
            variant="ghost"
            size="sm"
            className="absolute top-2 right-2 z-10 h-10 w-10 p-0 text-slate-300 hover:text-white hover:bg-red-500/20 hover:border-red-500/50 rounded-full border border-slate-600 bg-slate-800/80 backdrop-blur-sm"
          >
            <X className="w-5 h-5" />
          </Button>
