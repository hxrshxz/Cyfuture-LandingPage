    ...
    async (memo: string): Promise<SolanaActionResult> => {
      ...
      try {
        ...
        if (
          ...
          errorMessage =
            "Transaction simulation failed. This usually means the memo data is too large (80 byte limit) or contains invalid characters.";
        } else if (
          errorStr.includes("InvalidInstruction") ||
          errorStr.includes("invalid")
        ) {
          errorMessage =
            "Invalid transaction instruction. Check that memo data is properly formatted and under 80 bytes.";
        }
